...
from batch import BatchSubstructContext, BatchMasking, BatchAE,BatchSubstruct,BatchSubstruct_pos_neg,BatchSubstruct_pos_neg_pair
...
class DataLoaderSubstruct(torch.utils.data.DataLoader):
    r"""Data loader which merges data objects from a
    :class:`torch_geometric.data.dataset` to a mini-batch.
    Args:
        dataset (Dataset): The dataset from which to load the data.
    ...
    def __init__(self, dataset, batch_size=1, shuffle=True, **kwargs):
        super(DataLoaderSubstruct, self).__init__(
            dataset,
            batch_size,
            shuffle,
            collate_fn=lambda data_list: BatchSubstruct.from_data_list(data_list),
            **kwargs)

class DataLoaderSubstruct_pos_neg(torch.utils.data.DataLoader):
    r"""Data loader which merges data objects from a
    :class:`torch_geometric.data.dataset` to a mini-batch.
    Args:
        dataset (Dataset): The dataset from which to load the data.
    ...
    def __init__(self, dataset, batch_size=1, shuffle=True, **kwargs):
        super(DataLoaderSubstruct_pos_neg, self).__init__(
            dataset,
            batch_size,
            shuffle,
            # collate_fn=lambda data_list: BatchSubstruct_pos_neg.from_data_list(data_list),
            collate_fn=lambda data_list: BatchSubstruct_pos_neg.from_data_list(data_list),
            **kwargs)
